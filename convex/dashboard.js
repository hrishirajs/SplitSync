...
export const getUserBalances = query({
  handler: async (ctx) => {
    ...
    /* ───────────── expenses where user is involved (1:1 + groups) ───────────── */
    ...
    /* ───────────── settlements involving user (1:1 + groups) ───────────── */
      ...
      groups.map(async (group) => {
        ...
        settlements.forEach((settlement) => {
          if (settlement.paidByUserId === user._id) {
            // User paid someone (reduces what you owe)
            balance += settlement.amount;
            youOwe = Math.max(0, youOwe - settlement.amount);
          } else {
            // Someone paid the user (reduces what they owe you)
            balance -= settlement.amount;
