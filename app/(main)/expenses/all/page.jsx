...
// ─── Individual tab ───────────────────────────────────────────────────────────
function IndividualTab({ currentUser }) {
    const [search, setSearch] = useState("");

    const { data: contacts, isLoading: contactsLoading } = useConvexQuery(
        api.contacts.getAllContacts
    );
    const contactUsers = contacts?.users ?? [];

    const expenseQueries = contactUsers.map((contact) =>
        // eslint-disable-next-line react-hooks/rules-of-hooks
        useConvexQuery(api.expenses.getExpensesBetweenUsers, { userId: contact.id })
    );
    ...
    const allExpenses = useMemo(() => {
        ...
        expenseQueries.forEach((q, i) => {
            q.data?.expenses?.forEach((exp) => {
                if (!seen.has(exp._id)) {
                    seen.add(exp._id);
                    result.push({ ...exp, otherUser: contactUsers[i] });
                }
            });
        });
        return result.sort((a, b) => b.date - a.date);
    }, [expenseQueries, contactUsers]);
    ...
    const isLoading = contactsLoading || expenseQueries.some((q) => q.isLoading);
                ...
                <Card>
                    <CardContent className="p-2 divide-y divide-border/50">
                        {filtered.map((expense) => {
                            const iPaid = expense.paidByUserId === currentUser._id;
                            ...
                            return (
                                <Link
                                    key={expense._id}
                                    href={`/person/${expense.otherUser?.id}`}
                                ...
                                >
                                    ...
                                    <div className="text-right flex-shrink-0 ml-4">
                                        ...
                                        {myShare && (
                                            <p
                                                className={`text-xs mt-0.5 ${iPaid ? "text-green-600" : "text-red-500"}`}
                                            >
                                                {iPaid
...
// ─── Group tab ────────────────────────────────────────────────────────────────
function GroupTab({ currentUser }) {
    const [search, setSearch] = useState("");
    const [selectedGroupId, setSelectedGroupId] = useState("all");

    const { data: groupData, isLoading: groupsLoading } = useConvexQuery(
        api.groups.getGroupOrMembers,
        {}
    );
    const groups = groupData?.groups ?? [];

    const groupExpenseQueries = groups.map((group) =>
        // eslint-disable-next-line react-hooks/rules-of-hooks
        useConvexQuery(api.groups.getGroupExpenses, { groupId: group.id })
    );

    const allExpenses = useMemo(() => {
        const seen = new Set();
        const result = [];
        groupExpenseQueries.forEach((q, i) => {
            q.data?.expenses?.forEach((exp) => {
                if (!seen.has(exp._id)) {
                    seen.add(exp._id);
                    result.push({
    ...
    const filtered = useMemo(
        ...
        [allExpenses, search, selectedGroupId]
    );

    const isLoading =
        groupsLoading || groupExpenseQueries.some((q) => q.isLoading);
...
export default function AllExpensesPage() {
    ...
    const isLoading =
        balancesLoading || totalSpentLoading || monthlyLoading || !currentUser;
                ...
                <>
                    <AppPageHero
                        ...
                        stats={[
                            ...
                            {
                                value: `$${(balances?.youAreOwed ?? 0).toFixed(2)}`,
                                label: "owed to you",
                            },
                            {
                                value: `$${(balances?.youOwe ?? 0).toFixed(2)}`,
                                label: "you owe",
                            },
                        ]}
                    />
                    ...
                    <Tabs defaultValue="individual">
                        ...
                        <TabsContent value="individual" className="mt-6">
                            <IndividualTab currentUser={currentUser} />
                        </TabsContent>

                        <TabsContent value="group" className="mt-6">
                            <GroupTab currentUser={currentUser} />
                        </TabsContent>
                    </Tabs>
                </>
            )}
